...
class EntityLock
{
    ...
    public static function generateErrorMessage($strTable, $intEntity, $objModule)
    {
        ...
        {
            $strName    =
                $objLock->editorType == EntityLock::EDITOR_TYPE_USER ? $objEditor->name : $objEditor->firstname . ' ' . $objEditor->lastname;
    ...
    public static function generateUnlockForm($strTable, $objEntity, $objLock, $objModule, $arrTokens = [])
    {
        ...
        if ($blnIsSubmitted && \Input::get('act') == EntityLock::ACT_UNLOCK && $objLock->id == \Input::post('lock'))
        {
            ...
            {
                static::sendLockDeletionNotification(
                    $objModule->lockDeletionNotification,
                    $strTable,
                    $objEntity,
                    $objFormerEditor,
                    $arrTokens
            ...
            \Controller::redirect(Url::removeQueryString(['act']), $objTemplate->action);
        }

        return $objTemplate->parse();
    }

    public static function sendLockDeletionNotification(
        $intNotificationMessage,
        $strTable,
        \Model $objEntity,
        \MemberModel $objFormerEditor,
        $arrTokens = []
    ) {
    ...
    public static function generateTokens($strTable, \Model $objEntity, \MemberModel $objFormerEditor)
    {
        ...
        $arrTokens['salutation_former_editor'] = Salutations::createSalutation($GLOBALS['TL_LANGUAGE'], $objFormerEditor);
